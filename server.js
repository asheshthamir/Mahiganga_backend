...
// Database connection setup with REQUIRED SSL configuration
const pool = new Pool({
...
app.get('/api/vehicles/:id', async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).send('Server error fetching single vehicle');
  }
});
...
app.delete('/api/vehicles/:id', async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).send('Server error deleting vehicle');
  }
});
